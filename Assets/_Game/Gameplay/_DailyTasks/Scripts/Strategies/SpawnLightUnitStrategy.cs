...
{
    public class SpawnLightUnitStrategy : IDailyTaskStrategy
    {
        ...
        {
            if (unit.Type == UnitType.Light && !_model.IsReady)
            {
        ...
        private void SetAdditionalDescription()
        {
            var additionalDescription = TimelineConfig
                .GetAgeRelatedWarrior(TimelineState.TimelineId, TimelineState.AgeId, UnitType.Light).Name;

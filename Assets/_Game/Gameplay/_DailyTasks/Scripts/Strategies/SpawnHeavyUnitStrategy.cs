...
{
    public class SpawnHeavyUnitStrategy : IDailyTaskStrategy
    {
        ...
        {
            if (unit.Type == UnitType.Heavy && !_model.IsReady)
            {
        ...
        private void SetAdditionalDescription()
        {
            var additionalDescription = TimelineConfig
                .GetAgeRelatedWarrior(TimelineState.TimelineId, TimelineState.AgeId, UnitType.Heavy).Name;
